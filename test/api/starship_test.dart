...
void main() {
  group("Starship APIs", () {
    setUpAll(() async {
      ...
      final File file = File("test_resources/starship.json");
    ...
    test(
        "When requesting for get starship by id 1, return successful response by checking name is not empty",
        () async {
      final service = ApiService(getIt.get<ApiClient>());

      final data = await service.getStarship("1").single;
